// admin-auth.js - سیستم احراز هویت مدیر
...
class AdminAuth {
    ...
    setSession() {
        const session = {
            username: this.data.adminAuth.username,
    ...
    login(username, password) {
        if (username !== this.data.adminAuth.username) {
            return { success: false, message: '❌ نام کاربری اشتباه است.' };
        }
        
        if (password !== this.data.adminAuth.password) {
    ...
    changePassword(oldPassword, newPassword) {
        if (oldPassword !== this.data.adminAuth.password) {
        ...
        this.data.adminAuth.password = newPassword;
        this.data.adminAuth.lastChanged = new Date().toISOString();
        this.saveData();
        return { success: true, message: '✅ رمز عبور با موفقیت تغییر کرد!' };
    }
    
    resetPassword() {
        this.data.adminAuth.password = this.DEFAULT_PASSWORD;
        this.data.adminAuth.lastChanged = new Date().toISOString();
        this.saveData();
